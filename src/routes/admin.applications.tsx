...
function ApplicationsPage() {
  ...
  async function admit(id: string) { try { const result = await convert({ data: { application_id: id } }); toast.success(`Student created: ${result.admission_number}. Temporary password: ${result.temporary_password}`); queryClient.invalidateQueries({ queryKey: ["admin-applications"] }); } catch (error) { toast.error((error as Error).message); } }
