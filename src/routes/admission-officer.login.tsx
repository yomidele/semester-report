...
export const Route = createFileRoute("/admission-officer/login")({
  head: () => ({ meta: [{ title: "Admission Officer Sign In — School Portal" }] }),
  component: AdmissionOfficerLogin,
});

function AdmissionOfficerLogin() {
  const navigate = useNavigate();
  const { session, loading: authLoading } = useAuthSession();
  const { isAdmissionOfficer, isSuperAdmin, loading: roleLoading } = useRole();
  ...
  }, [authLoading, roleLoading, session, isAdmissionOfficer, isSuperAdmin, navigate]);
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-10">
        <Card className="w-full max-w-md tsu-shadow">
          <CardHeader>
            <CardTitle className="font-serif text-2xl">Admission Officer Sign In</CardTitle>
