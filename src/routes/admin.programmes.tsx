...
export const Route = createFileRoute("/admin/programmes")({
  head: () => ({ meta: [{ title: "Classes & Arms — Super Admin" }] }),
...
function Classes & ArmsPage() {
  ...
  const filteredClasss = departments.data?.filter((department) => department.faculty_id === form.faculty_id) ?? [];
  return <div className="space-y-6">
    <div><h2 className="font-serif text-2xl font-bold">Classes & Arms</h2><p className="text-sm text-muted-foreground">Create and maintain the programmes shown on the public site.</p></div>
    <Card><CardHeader><CardTitle className="text-base">{form.id ? "Edit class arm" : "Add class arm"}</CardTitle></CardHeader><CardContent><form className="grid gap-4 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); if (!form.faculty_id || !form.department_id || !form.name.trim() || !form.code.trim() || !form.award.trim()) return; save.mutate(); }}>
      ...
      <SelectField label="School" value={form.faculty_id} onChange={(value) => { set("faculty_id", value); set("department_id", ""); }} options={faculties.data?.map((item) => [item.id, item.name] as const) ?? []} /><SelectField label="Class" value={form.department_id} onChange={(value) => set("department_id", value)} options={filteredClasss.map((item) => [item.id, item.name] as const)} />
      ...
      <div className="flex items-end gap-5 pb-2"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.uses_gpa} onChange={(event) => set("uses_gpa", event.target.checked)} /> Uses Term Average</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.is_active} onChange={(event) => set("is_active", event.target.checked)} /> Active</label></div>
