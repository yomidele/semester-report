...
export const Route = createFileRoute("/exam-officer/login")({
  head: () => ({ meta: [{ title: "Exam Officer Sign In — School Portal" }] }),
  component: ExamOfficerLogin,
});

function ExamOfficerLogin() {
  const navigate = useNavigate();
  const { session, loading: authLoading } = useAuthSession();
  const { isExamOfficer, isSuperAdmin, loading: roleLoading } = useRole();
  ...
  }, [authLoading, roleLoading, session, isExamOfficer, isSuperAdmin, navigate]);
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-10">
        <Card className="w-full max-w-md tsu-shadow">
          <CardHeader>
            <CardTitle className="font-serif text-2xl">Exam Officer Sign In</CardTitle>
            <CardDescription>Accounts are created by the Super Admin.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSignIn} className="space-y-4">
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                <Link to="/admission-officer/login" className="hover:underline">Admission Officer →</Link>
