...
function Page() {
  ...
  const teachers = useQuery({ queryKey: ["dept-teachers"], queryFn: async () => (await supabase.from("lecturers").select("id, full_name").order("full_name")).data ?? [] });
  const subjects = useQuery({ queryKey: ["dept-subjects"], queryFn: async () => (await supabase.from("courses").select("id, code, title, level, semester").order("level").order("code")).data ?? [] });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Assign teachers to subjects for a session and semester.</p>
      </div>
      <Card>
